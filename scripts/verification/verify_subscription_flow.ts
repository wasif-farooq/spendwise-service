...
async function verify() {
    ...
    const workspaceId = '123e4567-e89b-12d3-a456-426614174000'; // UUID
    ...
    await db.query(`INSERT INTO workspaces (id, name, slug, owner_id, created_at, updated_at) VALUES ($1, $2, $3, $4, NOW(), NOW())`,
        [workspaceId, 'Test Workspace Subscription', `test-workspace-${Date.now()}`, userId]);

    try {
        // 2. Subscribe to Free
        console.log('Subscribing to Free plan...');

        await subService.subscribe(userId, 'free', { provider: 'manual', subscriptionId: 'sub-free-001' });

        let currentSub = await subService.getCurrentSubscription(userId);
        ...
        await subService.upgrade(userId, 'pro');

        currentSub = await subService.getCurrentSubscription(userId);
        console.log('Current Subscription:', currentSub?.planId);
        ...
        const subAfterPlanChange = await subService.getCurrentSubscription(userId);
